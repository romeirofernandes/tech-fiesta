...
export default function CreateAnimalScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Save local data (including imageUri for display)
      // Note: We store local URI in imageUrl field for offline display
      ...
      const animalData = {
        ...
        farmId,
        tempId,
        imageUri, // Passed to sync service
      };

      db.runSync(
        `INSERT INTO animals (id, name, rfid, species, breed, gender, age, ageUnit, farmId, farmName, farmLocation, imageUrl, createdAt, updatedAt, syncStatus, tempId)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          tempId,
          animalData.name,
          ...
          imageUri || null, // Store local URI for immediate display
          now,
          now,
          'pending',
          tempId,
      ...
      await syncService.addToQueue('CREATE', 'animals', animalData);
      ...
      >
        ...
        >
          ...
          <View className="px-5 pt-3 pb-2 flex-row items-center justify-between">
            ...
            <Text className="text-lg font-bold text-foreground">Add Animal</Text>
            <View className="w-16" />
          </View>
            ...
            <View className="w-32 h-32 rounded-3xl bg-muted/50 items-center justify-center border-2 border-dashed border-border overflow-hidden relative">
              {imageUri ? (
                ...
                <View className="items-center justify-center p-4">
                  ...
                  <Text className="text-[10px] text-muted-foreground text-center">Add Photo</Text>
                </View>
              )}
            </View>
