...
export default function RegisterScreen() {
  ...
  const syncWithBackend = async (
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/farmers/auth`, {
        ...
        body: JSON.stringify({ firebaseUid, fullName, email, phoneNumber }),
      });
  ...
  const handleEmailRegister = async () => {
    ...
    try {
      ...
      await syncWithBackend(user.uid, fullName, email, null);
    } catch (error: any) {
      console.error(error);
  ...
  const handleSendOtp = async () => {
    ...
    try {
      Alert.alert(
        ...
        'Phone authentication on mobile requires additional Firebase setup with reCAPTCHA verification. Please use email registration or configure Firebase phone auth in your Firebase console.',
        [{ text: 'OK' }]
      );
      setLoading(false);
    } catch (error: any) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      await syncWithBackend(user.uid, fullName, null, normalized);
    } catch (error: any) {
      console.error(error);
